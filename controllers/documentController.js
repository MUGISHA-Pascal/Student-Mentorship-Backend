...
export const createCourseWithDocument = async (req, res) => {
  const { coachId, courseName, description, careerId } = req.body;
  console.log("career id", careerId);
  if (!coachId || !courseName) {
    return res
      .status(400)
      .json({ message: "Coach ID and Course name are required" });
  }
...
export const deleteCourseAndDocuments = async (req, res) => {
  ...
  try {
    ...
    if (error.code === "P2025") {
      ...
      return res
        .status(404)
        .json({ message: "Course or related documents not found." });
    }
